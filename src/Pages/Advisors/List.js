...
export default function List(props) {
    ...
    const tableHeads = ["FULL NAME", "CALL HISTORY", "Card Number", "CALL RESPONSE", "RM Name", "REGISTERED DATE"];

    const tableElements = membersData && membersData.length > 0 ?
        membersData.map(data => ([
            <Link
                to={`/advisor/details/${data.EmployeeId}`}
            ...
            <>
                <button
                    ...
                    type="button"
                    onClick={() => handleCallLog(data.EmployeeId)}
                >
            ...
            <p className="text-center">
                {data.CreatedDate ? formatDate(data.CreatedDate).slice(0, 11) : ""}
            </p>,
    ...
    useEffect(() => {
        ...
        const getMemberTypes = async () => {
            const fetchMemberTypes = await fetchData('UserRoles/all', { skip: 0, take: 0 });
    ...
    }, [selectedStates, selectedDistricts, selectedResponse, selectedMandals, selectedNames,
        selectedNumbers, selectedRegionalManager, selectedRouteMap, selectedVillages, livelongStatusFilter, paStatusFilter]);

    useEffect(() => {
        getDistributorData();
    }, [userRoleId, filterCriteria, currentPage, perPage]);
    ...
    const fetchCallHistoryData = async () => {
        setLoading(true);
        try {
            const response = await fetchAllData(`lambdaAPI/CallHistory/GetAllCallHistoryByEmployeeId/${callLogMemberId}`);
    ...
    const getDistributorCountData = async () => {
        setLoading(true);
        try {
            const distributorCountData = await fetchData(`CommonRowCount/GetTableRowCount`, {
    ...
    const getDistributorData = async () => {
        ...
        try {
            ...
            if (filterCriteria.length > 0) {
                distributorData = await fetchData("lambdaAPI/Employee/GetMultipleDataByFilter", {
                    skip,
                    take,
                    filter: filterCriteria
                });
            } else {
                distributorData = await fetchData("lambdaAPI/Employee/GetAdvisors", { skip, take, userRoleId });
            }
    ...
    const handleExcelDownload = async () => {
        setLoading(true)
        await downloadExcelData('advisorlist', totalCount, perPage, currentPage, fetchData, filterCriteria, setLoading, userRoleId);
    };

    const handleCSVDownload = async () => {
        setLoading(true)
        await downloadCSVData('advisorlist', totalCount, perPage, currentPage, fetchData, filterCriteria, setLoading, userRoleId);
    };
    ...
    const handleInputChange = async (event) => {
        ...
        if (value.length >= 2) {
            ...
            try {
                const suggestionData = await fetchData("lambdaAPI/Employee/GetMultipleDataByNameAndMobileNum", {
                    skip: 0,
                    take: perPage,
                    filter: filterCriteria
                });
    ...
    const handleSuggestionClick = (suggestion) => {
        navigate(`/advisor/details/${suggestion.EmployeeId}`);
    };
    ...
    const applyFilter = async () => {
        ...
        try {
            ...
            const filterData = await fetchData("lambdaAPI/Employee/GetMultipleDataByFilter", {
                skip: 0,
                take: perPage,
                filter: filterCriteria
            })
                                ...
                                {callHistory.map((call, index) => (
                                    <li key={index} className="timeline-item timeline-item-transparent">
                                        ...
                                        <div className="timeline-event">
                                            ...
                                            {call.DateToBeVisited !== '0001-01-01T00:00:00' && call.DateToBeVisited !== null && (
                                                <>
                                                    <div className="timeline-header mb-1 mt-1">
                                            ...
                                            {call.RequestCallBack !== '0001-01-01T00:00:00' && call.RequestCallBack !== null && (
                                                <>
                                                    <div className="timeline-header mb-1 mt-1">
    ...
    const handleRemoveFilter = (filterType) => {
        ...
        // Trigger refetch of data with updated filters
        getDistributorData();
    };
                                    ...
                                    <div className="modal-body">
                                        <div className="accordion-body">
                                            <div className="row mb-2">
                                                <div className="col-lg-12 order-1 order-md-0">
