...
export default function AdvisorPersonalDetails(props) {
  ...
  const [selectedRM, setSelectedRM] = useState(profile?.JoinedBy || "");
  const [selectedReporting, setSelectedReporting] = useState(profile?.ReportingId || "");
  const [isEditingRM, setIsEditingRM] = useState(false);
  const [isEditingReporting, setIsEditingReporting] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setCardLoading(true);
        const response = await fetchAllData(
          `lambdaAPI/OHOCards/GetMemberCardByMemberId/${customerId}`
        );
  ...
  const fetchProductsData = async () => {
    try {
      ...
      const response = await fetchAllData(`lambdaAPI/Customer/GetMemberProducts/${customerId}`);
  ...
  const fetchMemberData = async () => {
    ...
    try {
      ...
      setMemberDetails(response[0]);
      setCustomerId(response[0].CustomerId);
      setLoading(false);
    } catch (error) {
      console.error("Error fetching data:", error);
      setLoading(false);
  ...
  const confirmhandleDelete = async () => {
    try {
      const response = await fetchDeleteData(`lambdaAPI/Employee/delete/${Id}`);
  ...
  const fetchCallHistoryData = async () => {
    ...
    try {
      ...
      const response = await fetchAllData(
        `lambdaAPI/CallHistory/GetAllCallHistoryByEmployeeId/${Id}`
      );
  ...
  const handleRMUpdate = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        setTimeout(() => {
          window.location.reload();
        }, 500); // Reloads the page after 1 second
      }

    } catch (error) {
      console.error("Error updating RM:", error);
      alert("Failed to update RM.");
    } finally {
      setLoading(false);
  ...
  const handleReportingUpdate = async () => {
    if (isFetching.current) return;
    isFetching.current = true;
    ...
    try {
      ...
      const updateData = [
        {
          columnName: "ReportingTo",
          columnValue: selectedReporting,
          tableName: "Employee",
          tableId: Id,
        },
      ];
      ...
      if (response) {
        alert("Reporting updated successfully!");
        setIsEditingRM(false);
      ...
      alert("Failed to update Reporting.");
    } finally {
      setLoading(false);
      isFetching.current = false;
    }
  ...
  useEffect(() => {
    if (addFormData.StateId) {
      getDistricts(addFormData.StateId);
    }
  }, [addFormData.StateId]);
  ...
  const getRegionalManagers = async (rmId = null) => {
    ...
    try {
      if (rmId) {
        ...
        setSelectedReporting(userData?.UserId)
        setProfile((prev) => ({
          ...prev,
          RMName: userData?.UserName || "Not exist",
        }));
      } else {
  ...
  const handleFileSubmit = async (e) => {
    ...
    formData.append("MemberId", customerId);
  ...
  const getDistributorCards = async () => {
    try {
      ...
      const distributorCardsData = await fetchAllData(
        `lambdaAPI/OHOCards/GetByMemberId/${Id}`
      );
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const response = await fetchData(
        "lambdaAPI/OHOCards/AssignCardsToDistributor",
        payload
      );
                        ...
                        <div className="d-flex align-items-center flex-wrap gap-2">
                          ...
                          {profile.CreatedDate && (
                            <span className="badge bg-light text-secondary fw-normal px-3 py-2 rounded-pill">
                              ...
                              Joined {moment(profile.CreatedDate).format("DD MMM YYYY")}
                            </span>
                          )}
                        </div>
                      </div>
              ...
              <div className="row">
                ...
                <div className="col-md-4">
                  <h5 className="d-flex align-items-center mb-4">
                    <span
                      className="icon-wrapper d-flex align-items-center justify-content-center rounded-circle bg-primary text-white me-3"
                      style={{ width: "38px", height: "38px" }}
                    >
                  ...
                  <div className="position-relative">
                    ...
                    <div className="d-flex align-items-start mb-3">
                      ...
                      <div className="w-100">
                        <div className="text-muted mb-1">RM Name</div>
                    ...
                    <div className="d-flex align-items-start">
                      <i className="bx bx-id-card text-primary fs-4 me-3 mt-1"></i>
                      <div className="w-100">
                        <div className="text-muted mb-1">Reporting To</div>
                        <div className="mt-2 d-flex align-items-center gap-2">
                          {!isEditingReporting ? (
                            <>
                              <span className="fw-semibold fs-5">
                                {profile.ReportingName || <span className="text-danger">Not assigned</span>}
                              </span>
                              <button
                                className="btn btn-link btn-sm text-primary p-0 ms-3"
                                onClick={() => {
                                  getRegionalManagers();
                                  setIsEditingReporting(true);
                                }}
                              >
                                <i className="bx bx-edit-alt fs-5"></i>
                              </button>
                            ...
                            <div className="d-flex flex-column w-100 gap-2">
                              <select
                                ...
                                onChange={(e) => setSelectedReporting(e.target.value)}
                              >
                              ...
                              <div className="d-flex gap-2 mt-2">
                                <button className="btn btn-success py-2 px-3 flex-grow-1" onClick={handleReportingUpdate}>
                                  <i className="bx bx-check me-1"></i> Save
                                </button>
                                <button className="btn btn-outline-secondary py-2 px-3" onClick={() => setIsEditingReporting(false)}>
                                  <i className="bx bx-x"></i> Cancel
                                </button>
                              </div>
                            </div>
                    ...
                    <div className="input-group">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setAddFormData({ ...addFormData, Pincode: numericValue });
                        }}
                      />
                    </div>
                  </div>
      ...
      {activeSection === "product" && (
        <div className="col-md-12 mb-4">
          <div className="card shadow-sm">
            ...
            {/* {!showIndividualProducts && !selectedProduct && products && products.length > 0 && (
              <div className="px-3 pt-3">
                <div className="input-group mb-3">
                  ...
                  <ul className="dropdown-menu dropdown-menu-end">
                    <li><button className="dropdown-item">All Products</button></li>
                    <li><button className="dropdown-item">Combo Products</button></li>
                    <li><button className="dropdown-item">Individual Products</button></li>
                    <li><hr className="dropdown-divider" /></li>
                    <li><button className="dropdown-item">Active</button></li>
                    <li><button className="dropdown-item">Expiring Soon</button></li>
                  </ul>
                </div>
              </div>
            )} */}
          ...
          {distributorCards && distributorCards.length > 0 && (
            <div className="card shadow-sm border-0 rounded-lg">
              <div className="card-header d-flex justify-content-between align-items-center bg-white py-3 border-0">
                <h5 className="fw-bold m-0 text-primary">Advisor Cards</h5>
            ...
            {purchaseProduct ? (
              <>
                {purchaseProduct ? (
                          ...
                          <span>
                            {purchaseProduct.IsFree ? (
                              <>
                                ...
                                >
                                  {
                                    purchaseProduct.InsurancePremiums[0]
                                      .TotalAmount
                                  }
                                </span>{" "}
                                Free
                  ...
                  <div>
                    <h5 className="text-danger fw-semibold text-center">
