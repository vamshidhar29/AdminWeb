...
export default function AdvisorTabs() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await fetchAllData(`MemberDependent/GetByMemberId/${id.Id}`);
                setFamilyMembesLength(response.length);
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            if (!bucketUrlConfig) {
                console.error("Membership download URL configuration not found.");
                alert("Unable to download membership. Configuration missing.");
                return;
            }
            ...
            if (status && data) {
                ...
                console.error("File download failed or data is missing. Status:", status, "Message:", message);
                alert("Unable to download membership. " + message);
            }
        } catch (error) {
            console.error('Error downloading membership file:', error);
            alert("An error occurred while trying to download the membership.");
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="col-md-12">
                    <ul className="nav nav-pills flex-column flex-sm-row mb-4">
                        ...
                        <li className="nav-item">
                            ...
                            >
                                <i className='bx bx-grid-alt'></i> OHOCards Advisor
                            </button>
                        </li>
                        <li className="nav-item">
                            <button
                ...
                <>
                            ...
                            {selectedProduct ? (
                                <>
                                    <div className="d-flex flex-row mb-2">
                                        ...
                                        {selectedProduct.PoliciesPolicyDocument !== null && (
                                            <button className="btn bg-primary h-25 ms-2">
                                                <a
                                                    href={`${paUrl}${selectedProduct.PoliciesPolicyDocument}`}
                    ...
                    {activeTab === 'ohoCardsDistributor' && (
                        <>
                            ...
                            <h2 style={CardcustomStyles.header}>Advisor Cards</h2>
                            ...
                            {loading ? (
                                ...
                                <div style={CardcustomStyles.errorContainer}>
                                    <p style={CardcustomStyles.errorText}>No advisor cards found.</p>
                                </div>
                            ) : (
                                    ...
                                    <table style={CardcustomStyles.table}>
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={CardcustomStyles.th}>Advisor Name</th>
